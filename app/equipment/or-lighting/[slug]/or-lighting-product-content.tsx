...
const copy = {
  en: {
    ...
    capabilitiesHeading: "Core capabilities for modern surgical workflows",
    deploymentHeading: "Adaptive deployment options",
    deploymentIntro: (name: string) =>
      `${name} can be configured for different operating room formats and clinical workflows while maintaining consistent lighting quality.`,
    exploreOther: "Explore other OR lighting solutions",
    bottomTitle: "Need support selecting the right surgical lights?",
    bottomText:
      "We can evaluate your procedure mix, room geometry, and technical constraints to define the optimal lighting concept for your operating suites.",
  },
  ua: {
    equipment: "Обладнання",
    category: "Операційне освітлення",
    back: "Назад до операційного освітлення",
    ...
    capabilitiesHeading: "Ключові можливості для сучасних хірургічних процесів",
    deploymentHeading: "Варіанти впровадження",
    deploymentIntro: (name: string) =>
      `${name} може конфігуруватися для різних форматів операційних і клінічних сценаріїв, зберігаючи стабільну якість освітлення.`,
    exploreOther: "Переглянути інші рішення для операційного освітлення",
    bottomTitle: "Потрібна допомога з вибором хірургічних світильників?",
    bottomText:
      "Ми можемо оцінити профіль процедур, геометрію приміщення та технічні обмеження, щоб визначити оптимальну концепцію освітлення для ваших операційних.",
  },
} as const

type Props = {
  product: OrLightingProduct
  prevProduct: OrLightingProduct | null
  nextProduct: OrLightingProduct | null
}

export function OrLightingProductContent({ product, prevProduct, nextProduct }: Props) {
      ...
      <section className="relative overflow-hidden border-b border-border/50 py-16 md:py-20">
        ...
        <Container className="relative">
          <nav className="mb-4 flex flex-wrap items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            ...
            <Link href="/equipment/or-lighting" className="transition-colors hover:text-primary">
              {t.category}
            </Link>
            <ChevronRight className="h-3.5 w-3.5" />
          ...
          <div className="flex flex-wrap gap-2">
            {prevProduct && (
              <Button asChild variant="outline" size="sm" className="rounded-lg border-border/50 hover:border-primary/40">
                <Link href={`/equipment/or-lighting/${prevProduct.slug}`}>
                  <ArrowLeft className="h-4 w-4" />
                  {prevProduct.name}
                </Link>
              </Button>
            ...
            {nextProduct && (
              <Button asChild variant="outline" size="sm" className="rounded-lg border-border/50 hover:border-primary/40">
                <Link href={`/equipment/or-lighting/${nextProduct.slug}`}>
                  {nextProduct.name}
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
