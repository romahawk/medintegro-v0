...
export default async function BedHeadUnitProductPage({
  ...
  const currentIndex = bedHeadUnitProducts.findIndex((item) => item.slug === product.slug)
  const prevProduct = currentIndex > 0 ? bedHeadUnitProducts[currentIndex - 1] : null
  const nextProduct =
    currentIndex < bedHeadUnitProducts.length - 1 ? bedHeadUnitProducts[currentIndex + 1] : null

  const capabilityIcons = [Layers, Shield, Settings2]
  const heroImages = {
    main: {
      src: getBedHeadUnitImage(product.slug, "hero-main.jpg", product.heroImages.main.src),
      alt: product.heroImages.main.alt,
    },
    secondary1: {
      src: getBedHeadUnitImage(product.slug, "hero-secondary-1.jpg", product.heroImages.secondary1.src),
      alt: product.heroImages.secondary1.alt,
    },
    secondary2: {
      src: getBedHeadUnitImage(product.slug, "hero-secondary-2.jpg", product.heroImages.secondary2.src),
      alt: product.heroImages.secondary2.alt,
    },
  }
  const deploymentImages = [
    {
      src: getBedHeadUnitImage(product.slug, "deploy-1.jpg", product.deploymentImages[0].src),
      alt: product.deploymentImages[0].alt,
    },
    {
      src: getBedHeadUnitImage(product.slug, "deploy-2.jpg", product.deploymentImages[1].src),
      alt: product.deploymentImages[1].alt,
    },
    {
      src: getBedHeadUnitImage(product.slug, "deploy-3.jpg", product.deploymentImages[2].src),
    ...
    <>
      ...
      <section id="capabilities" className="relative overflow-hidden border-y border-border/50 py-16 md:py-24">
        ...
        <Container className="relative">
          <h2 className="text-balance text-2xl font-semibold tracking-tight text-foreground md:text-3xl">
            Core capabilities for bedside care environments
          </h2>
      ...
      <section id="deployment" className="py-16 md:py-24">
        <Container>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-muted-foreground md:text-base">
            {product.name} is suited to a range of hospital ward and care environments
            where reliable, accessible bedside media infrastructure is required.
          </p>
      ...
      <section className="relative overflow-hidden border-y border-border/50 py-10">
        <Container className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm text-muted-foreground">Explore other bed head units</div>
          <div className="flex flex-wrap gap-2">
            {prevProduct && (
              <Button asChild variant="outline" size="sm" className="rounded-lg border-border/50 hover:border-primary/40">
                <Link href={`/equipment/icu-infrastructure/bed-head-units/${prevProduct.slug}`}>
                  <ArrowLeft className="h-4 w-4" />
                  {prevProduct.name}
                </Link>
              </Button>
            ...
            {nextProduct && (
              <Button asChild variant="outline" size="sm" className="rounded-lg border-border/50 hover:border-primary/40">
                <Link href={`/equipment/icu-infrastructure/bed-head-units/${nextProduct.slug}`}>
                  {nextProduct.name}
                  <ArrowRight className="h-4 w-4" />
                </Link>
              </Button>
      ...
      <section className="relative overflow-hidden border-t border-border/50 py-20 md:py-24">
        ...
        <Container className="relative flex flex-col items-center gap-6 text-center">
          <h2 className="max-w-xl text-balance text-2xl font-semibold tracking-tight text-foreground md:text-3xl">
            Need support selecting the right bed head unit?
          </h2>
          <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground md:text-base">
            We can evaluate your ward specification, media requirements, and aesthetic brief
            to define the optimal bed head unit configuration for each care environment.
          </p>
          <Button asChild size="lg" className="gap-2 rounded-xl glow-cyan">
            <Link href="/contact">
              Request Quote
